...
def affine_relu_bn_forward(x, w, b, gamma, beta, bn_param):
  """
  Convenience layer that performs an affine transform followed by a ReLU and batch norm
  ...
  out, bn_cache = batchnorm_forward(a2, gamma, beta, bn_param)
...
def affine_relu_bn_backward(dout, cache):
  ...
  da2, dgamma, dbeta = batchnorm_backward(dout, bn_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(len(hidden_dims)):
        level = str(i+1)
        self.params['W'+level] = weight_scale * np.random.randn(prev_dim, hidden_dims[i])
        self.params['b'+level] = np.zeros(hidden_dims[i])
        if self.use_batchnorm:
          self.params['gamma' + level] = np.ones(hidden_dims[i])
          self.params['beta' + level] = np.zeros(hidden_dims[i])
        prev_dim = hidden_dims[i]
    level = str(len(hidden_dims) + 1)
    self.params['W'+level] = weight_scale * np.random.randn(prev_dim, num_classes)
    self.params['b'+level] = np.zeros(num_classes)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for k, v in self.params.iteritems():
      if (isinstance(self.params[k], np.ndarray)):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(num_layers-1):
        level = str(i+1)
        W = self.params['W' + level]
        b = self.params['b' + level]
        if use_batchnorm:
          gamma = self.params['gamma' + level]
          beta = self.params['beta' + level]
          values, cache[i+1] = affine_relu_bn_forward(values, W, b, gamma, beta, \
            self.bn_params[i])
        else: 
          values, cache[i+1] = affine_relu_forward(values, W, b)
        if use_dropout:
          values, dropout_cache = dropout_forward(values, self.dropout_param)
          cache[i+1] += ((dropout_cache),)
       
    level = str(num_layers)
    W = self.params['W' + level]
    b = self.params['b' + level] 
    # Forward into last layer
    scores, cache[num_layers] = affine_forward(values, W, b)
    ...
    for i in xrange(num_layers-1, 0, -1):
        ...
        if use_batchnorm:
          dx, dw, db, dgamma, dbeta = affine_relu_bn_backward(dx, cache[i])
          gammakey = 'gamma' + level
          betakey = 'beta' + level
          grads[gammakey] = dgamma
          grads[betakey] = dbeta
        else: 
